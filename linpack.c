...
// Changed return type of linpack
...
// New function added to count median
static REAL count_median (int n, REAL *array);

static void *mempool;

// Removed all unnecessary printfs
void main(void)
	...
	{
	fprintf(stderr, "Not enough memory available for given array size.\n\n");
	exit(1);
	}
  
    nreps=1;
	// Changed linpack behavior, so the loop is inside of it
...
static void linpack(long nreps,int arsize)
	...
	// Added array for results, size is arbitrary, but it will never be reached
	REAL results[1000];
	// Counter helper
	int results_size = 0;
    ...
    if (toverhead<0.)
        toverhead=0.;
	nreps*=2;
	results[results_size] = kflops;
	results_size++;
	}
	
	// Print median to stdout
	printf("%9.3f\n", count_median(results_size, results));
	
}

// Taken from: https://en.wikiversity.org/wiki/C_Source_Code/Find_the_median_and_mean
static REAL count_median(int n, REAL x[]) {
    REAL temp;
    int i, j;
